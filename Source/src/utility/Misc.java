...
public class Misc {
	 ...
	 * 			chance is true
	 */
	public static boolean chance(int percentageAmount)
	{
		if (random(99) < percentageAmount)
		{
			return true;
		}
		return false;
	...
	{
        for(int i = 0; i < Constants.offensiveLanguage.length; i++) 
        {
			if(chat.toLowerCase().contains(Constants.offensiveLanguage[i])) 
			{
				return true;
			}
        }
