...
public class SmithingInterface {
	...
	public static void showSmithInterface(Client player, int itemId) 
	{
		if (ItemAssistant.getItemAmount(player, 2347) == 0)
		{
			player.sendMessage("You need a hammer.");
			return;
		}
		if (itemId == 2349)
	...
	private static String GetForlvl(int i, Client c) {
		if (c.skillLevel[13] >= i)
			return "@whi@";
		
			return "@bla@";
	}

	private static String GetForBars(int i, int j, Client player) {
		if (ItemAssistant.playerHasItem(player, i, j))
		return "@gre@";
