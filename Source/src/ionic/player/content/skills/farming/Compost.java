...
public class Compost {
	...
	private void updateCompostBin(int index) {
		...
		player.getPA().checkObjectSpawn(finalObject, x, y, CompostBinLocations.forId(index).getObjectFace(), 10);
	}
	...
	public void fillCompostBin(int x, int y, final int organicItemUsed) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			@Override
			public void execute(CycleEventContainer container) {
				if (!player.stopPlayerSkill || !ItemAssistant.playerHasItem(player, organicItemUsed) || organicItemAdded[index] == 15) {
					container.stop();
					return;
				}
				organicItemAdded[index]++;
				player.startAnimation(832);
				ItemAssistant.deleteItem(player, organicItemUsed, 1);
	...
	public void retrieveCompost(final int index) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			@Override
			public void execute(CycleEventContainer container) {
				if (!player.stopPlayerSkill || !ItemAssistant.playerHasItem(player, BUCKET) && compostBins[index] != 350 || organicItemAdded[index] == 0) {
					container.stop();
					stop();
					return;
				}
				player.getPA().addSkillXP((int) (finalItem == COMPOST ? COMPOST_EXP_RETRIEVE : finalItem == SUPER_COMPOST ? SUPER_COMPOST_EXP_RETRIEVE : ROTTEN_TOMATOES_EXP_RETRIEVE), Constants.FARMING);
				if (compostBins[index] != 350) {
					ItemAssistant.deleteItem(player, BUCKET, 1);
				}
				ItemAssistant.addItem(player, finalItem, 1);
				player.startAnimation(832);
				organicItemAdded[index]--;
