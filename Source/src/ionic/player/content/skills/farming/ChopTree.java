...
public class ChopTree {
    ...
    public static boolean handleNest(Client player, int itemId){
        ...
        switch(itemId){
            case 5070:
                ...
                ItemAssistant.addItem(player, 5075, 1);
                ItemAssistant.addItem(player, 5076, 1);
                return true;
            case 5071:
                ItemAssistant.deleteItem(player, itemId, 1);
                ItemAssistant.addItem(player, 5075, 1);
                ItemAssistant.addItem(player, 5078, 1);
                return true;
            case 5072:
                ItemAssistant.deleteItem(player, itemId, 1);
                ItemAssistant.addItem(player, 5075, 1);
                ItemAssistant.addItem(player, 5077, 1);
                return true;
            case 5073:
	...
	public static int getAxeIndex(Client player) {
		for(int i = 0; i < axes.length; i++) {
			if(player.playerEquipment[Constants.WEAPON_SLOT] == (axes[i].getId())) {
				return i;
			}
		}
		for (int i = axes.length - 1; i >= 0; i--) {
			if (ItemAssistant.playerHasItem(player, axes[i].getId())) {
				if (player.skillLevel[Constants.WOODCUTTING] >= axes[i].getLevel()) {
					return i;
				}
			}
		}
