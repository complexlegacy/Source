...
public class Allotments {
	...
	public void doCalculations() {
		for (int i = 0; i < allotmentSeeds.length; i++) {
			if (allotmentStages[i] > 0 && allotmentStages[i] <= 3 && Server.getMinutesCounter() - allotmentTimer[i] >= 5) {
				allotmentStages[i]--;
...
//			diseaseChance[index] *= 2;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (allotmentData.getLevelRequired() > player.skillLevel[Constants.FARMING]) {
			Dialogue.sendStatement2(player, new String[] {"You need a farming level of " + allotmentData.getLevelRequired() + " to plant this seed."});
			return true;
		}
		...
		if (ItemAssistant.getItemAmount(player, allotmentData.getSeedId()) < allotmentData.getSeedAmount()) {
			Dialogue.sendStatement2(player, new String[] {"You need atleast " + allotmentData.getSeedAmount() + " seeds to plant here."});
			return true;
		}
		player.startAnimation(FarmingConstants.SEED_DIBBING);
		allotmentStages[allotmentFieldsData.getAllotmentIndex()] = 4;
		ItemAssistant.deleteItem(player, seedId, ItemAssistant.getItemSlot(player, seedId), allotmentData.getSeedAmount());
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				allotmentTimer[allotmentFieldsData.getAllotmentIndex()] = Server.getMinutesCounter();
				player.getPA().addSkillXP((int) allotmentData.getPlantingXp(), Constants.FARMING);
				container.stop();
			}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] == 0) {
					allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] = (int) (1 + (START_HARVEST_AMOUNT + Misc.random((END_HARVEST_AMOUNT + (player.playerEquipment[Constants.WEAPON_SLOT] == 7409 ? 15 : 0)) - START_HARVEST_AMOUNT)) * (1));
				}
				...
				ItemAssistant.addItem(player, allotmentData.getHarvestId(), 1);
				player.getPA().addSkillXP((int) allotmentData.getHarvestXp(), Constants.FARMING);
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] == 0) {
			Dialogue.sendStatement2(player, new String[] {"This is an allotment patch. The soil has not been treated.", "The patch needs weeding."});
		} else if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] == 3) {
			Dialogue.sendStatement2(player, new String[] {"This is an allotment patch. The soil has not been treated.", "The patch is empty and weeded."});
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4 < inspectData.getMessages().length - 2) {
						Dialogue.sendStatement2(player, inspectData.getMessages()[allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4]);
...
//		player.getSkillGuide().selected = 20;
	...
	public void handleFlowerProtection(int index) {
		...
		if (!allotmentWatched[index]) {// Misc.random(100) <= maxChance) {
			...
			} else if (Misc.random(100) <= maxChance){
