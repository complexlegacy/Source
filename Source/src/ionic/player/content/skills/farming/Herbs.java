...
public class Herbs {
	...
	public void doCalculations() {
		for (int i = 0; i < herbSeeds.length; i++) {
			if (herbStages[i] > 0 && herbStages[i] <= 3 && Server.getMinutesCounter() - herbTimer[i] >= 1) {
				herbStages[i]-=2;
...
//				herbState[index] = 1;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (herbData.getLevelRequired() > player.skillLevel[Constants.FARMING]) {
			Dialogue.sendStatement2(player, new String[] {"You need a farming level of " + herbData.getLevelRequired() + " to plant this seed."});
			return true;
		}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				herbTimer[herbFieldsData.getHerbIndex()] = Server.getMinutesCounter();
				player.getPA().addSkillXP((int) herbData.getPlantingXp(), Constants.FARMING);
				container.stop();
			}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (herbHarvest[herbFieldsData.getHerbIndex()] == 0) {
					herbHarvest[herbFieldsData.getHerbIndex()] = (int) (1 + (START_HARVEST_AMOUNT + Misc.random((END_HARVEST_AMOUNT + (player.playerEquipment[Constants.WEAPON_SLOT] == 7409 ? 5 : 0)) - START_HARVEST_AMOUNT)) * (1));
				}
				...
				ItemAssistant.addItem(player, herbData.getHarvestId(), 1);
				player.getPA().addSkillXP((int) herbData.getHarvestXp(), Constants.FARMING);
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (herbStages[herbFieldsData.getHerbIndex()] == 0) {
			Dialogue.sendStatement2(player, new String[] {"This is an herb patch. The soil has not been treated.", "The patch needs weeding."});
		} else if (herbStages[herbFieldsData.getHerbIndex()] == 3) {
			Dialogue.sendStatement2(player, new String[] {"This is an herb patch. The soil has not been treated.", "The patch is empty and weeded."});
