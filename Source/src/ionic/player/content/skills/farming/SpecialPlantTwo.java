...
public class SpecialPlantTwo {
	...
	public void doCalculations() {
		for (int i = 0; i < specialPlantSeeds.length; i++) {
			if (specialPlantStages[i] > 0 && specialPlantStages[i] <= 3 && Server.getMinutesCounter() - specialPlantTimer[i] >= 5) {
	...
	public boolean plantSeeds(int objectX, int objectY, final int seedId) {
		...
		if (specialPlantData.getLevelRequired() > player.skillLevel[Constants.FARMING]) {
			Dialogue.sendStatement2(player, new String[] {"You need a farming level of " + specialPlantData.getLevelRequired() + " to plant this seed."});
			return true;
		}
		...
		if (ItemAssistant.getItemAmount(player, specialPlantData.getSeedId()) < specialPlantData.getSeedAmount()) {
			Dialogue.sendStatement2(player, new String[] {"You need atleast " + specialPlantData.getSeedAmount() + " seeds to plant here."});
			return true;
		}
		player.startAnimation(FarmingConstants.SEED_DIBBING);
		specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] = 4;
		ItemAssistant.deleteItem(player, seedId, specialPlantData.getSeedAmount());
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.getPA().addSkillXP((int) specialPlantData.getPlantingXp(), Constants.FARMING);
				container.stop();
			}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (specialPlantState[specialPlantFieldsData.getSpecialPlantsIndex()] == 3) {
					...
					player.getPA().addSkillXP((int) specialPlantData.getCheckHealthXp(), Constants.FARMING);
					specialPlantState[specialPlantFieldsData.getSpecialPlantsIndex()] = 0;
					hasFullyGrown[specialPlantFieldsData.getSpecialPlantsIndex()] = false;
					specialPlantTimer[specialPlantFieldsData.getSpecialPlantsIndex()] = Server.getMinutesCounter() - specialPlantData.getGrowthTime();
				...
				ItemAssistant.addItem(player, specialPlantData.getHarvestId(), 1);
				player.getPA().addSkillXP((int) specialPlantData.getHarvestXp(), Constants.FARMING);
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 0) {
			Dialogue.sendStatement2(player, new String[] {"This is one of the special patches. The soil has not been treated.", "The patch needs weeding."});
		} else if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 3) {
			Dialogue.sendStatement2(player, new String[] {"This is one of the special patches. The soil has not been treated.", "The patch is empty and weeded."});
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4 < inspectData.getMessages().length - 2) {
						Dialogue.sendStatement2(player, inspectData.getMessages()[specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4]);
