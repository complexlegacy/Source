...
public class Bushes {
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (bushesData.getLevelRequired() > player.skillLevel[Constants.FARMING]) {
			Dialogue.sendStatement2(player, new String[] {"You need a farming level of " + bushesData.getLevelRequired() + " to plant this seed."});
			return true;
		}
		...
		if (ItemAssistant.getItemAmount(player, bushesData.getSeedId()) < bushesData.getSeedAmount()) {
			Dialogue.sendStatement2(player, new String[] {"You need atleast " + bushesData.getSeedAmount() + " seeds to plant here."});
			return true;
		}
		player.startAnimation(FarmingConstants.SEED_DIBBING);
		bushesStages[bushesFieldsData.getBushesIndex()] = 4;
		ItemAssistant.deleteItem(player, seedId, bushesData.getSeedAmount());
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				bushesTimer[bushesFieldsData.getBushesIndex()] = Server.getMinutesCounter();
				player.getPA().addSkillXP((int) bushesData.getPlantingXp(), Constants.FARMING);
				container.stop();
			}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (bushesState[bushesFieldsData.getBushesIndex()] == 3) {
					...
					player.getPA().addSkillXP((int) bushesData.getCheckHealthXp(), Constants.FARMING);
					bushesState[bushesFieldsData.getBushesIndex()] = 0;
				...
				ItemAssistant.addItem(player, bushesData.getHarvestId(), 1);
				player.getPA().addSkillXP((int) bushesData.getHarvestXp(), Constants.FARMING);
				bushesTimer[bushesFieldsData.getBushesIndex()] = Server.getMinutesCounter();
				int difference = bushesData.getEndingState() - bushesData.getStartingState();
				int growth = bushesData.getGrowthTime();
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (bushesStages[bushesFieldsData.getBushesIndex()] == 0) {
			Dialogue.sendStatement2(player, new String[] {"This is a bush patch. The soil has not been treated.", "The patch needs weeding."});
		} else if (bushesStages[bushesFieldsData.getBushesIndex()] == 3) {
			Dialogue.sendStatement2(player, new String[] {"This is a bush patch. The soil has not been treated.", "The patch is empty and weeded."});
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (bushesStages[bushesFieldsData.getBushesIndex()] - 4 < inspectData.getMessages().length - 2) {
						Dialogue.sendStatement2(player, inspectData.getMessages()[bushesStages[bushesFieldsData.getBushesIndex()] - 4]);
