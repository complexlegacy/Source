...
//					player.getDH().sendNpcChat("If you like, but I want " + payment[1] + " " + getFinalPaymentString(ItemAssistant.getItemName(payment[0]).toLowerCase()) + (getFinalPaymentString(ItemAssistant.getItemName(payment[0]).toLowerCase()).endsWith("s") ? "" : "s") + " for that.", player.getDH().UNINTERESTED);
			...
			if (ItemAssistant.getItemAmount(player, payment[0]) < payment[1]) {
			//TODO	Dialogue.sendNpcChat("Sorry, but you do not have the required items", "I need, for letting me take care of this patch", 0 , "Farming");
				...
				ItemAssistant.deleteItem(player, payment[0], payment[1]);
				//TODO player.getDH().sendNpcChat3("Here you go, I will be taking care of this patch", "as soon as it become diseased, I will cure it", "so you don't have to worry about it.", 0 , "Farming");
	...
	public static void chopDownTree(Client player, int npcId) {
		...
		if (ItemAssistant.getItemAmount(player, 995) < 200) {
			//TODO player.getDH().sendNpcChat2("I am sorry, but you do not have enough money", "to pay me to chop down this tree.", 0 , "Farming");
			
		} else {
			ItemAssistant.deleteItem(player, 995, 200);
			//TODO player.getDH().sendNpcChat2("There you go, I have chopped down your tree but I am ", "keeping the logs and roots as compensation.", 0 , "Farming");
...
//		player.getDH().sendNpcChat(farmingAdvices[Misc.random(farmingAdvices.length - 1)], player.getDH().HAPPY);
