...
public class WoodTrees {
	...
	public boolean plantSapling(int objectX, int objectY, final int saplingId) {
		...
		if (treeData.getLevelRequired() > player.skillLevel[Constants.FARMING]) {
			Dialogue.sendStatement2(player, new String[] {"You need a farming level of " + treeData.getLevelRequired() + " to plant this sapling."});
			return true;
		}
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.getPA().addSkillXP((int) treeData.getPlantingXp(), Constants.FARMING);
				container.stop();
			}
	...
	public boolean checkHealth(int objectX, int objectY) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.getPA().addSkillXP((int) treeData.getCheckHealthXp(), Constants.FARMING);
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (treeState[treeFieldsData.getTreeIndex()] == 1) {
			Dialogue.sendStatement2(player, new String[] {"This tree is diseased. Use secateurs to prune the area, ", "or clear the patch with a spade."});
			return true;
		} else if (treeState[treeFieldsData.getTreeIndex()] == 2) {
			Dialogue.sendStatement2(player, new String[] {"This tree is dead. You did not cure it while it was diseased.", "Clear the patch with a spade."});
			return true;
		} else if (treeState[treeFieldsData.getTreeIndex()] == 6) {
			Dialogue.sendStatement2(player, new String[] {"This is a tree stump, to remove it, use a spade on it", "to recieve some roots and clear the patch."});
			return true;
		}
		if (treeStages[treeFieldsData.getTreeIndex()] == 0) {
			Dialogue.sendStatement2(player, new String[] {"This is a tree patch. The soil has not been treated.", "The patch needs weeding."});
		} else if (treeStages[treeFieldsData.getTreeIndex()] == 3) {
			Dialogue.sendStatement2(player, new String[] {"This is a tree patch. The soil has not been treated.", "The patch is empty and weeded."});
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (treeStages[treeFieldsData.getTreeIndex()] - 4 < inspectData.getMessages().length - 2) {
						Dialogue.sendStatement2(player, inspectData.getMessages()[treeStages[treeFieldsData.getTreeIndex()] - 4]);
	...
	public boolean cut(final int x, final int y) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			final ChopTree.Tree tree = ChopTree.getTree(object);
			@Override
			public void execute(CycleEventContainer container) {
				...
				player.startAnimation(emoteId);
				if (Skill.skillCheck(player.skillLevel[Constants.WOODCUTTING], treeLevel, axeChance*20)) {
					ItemAssistant.addItem(player, ChopTree.getTree(object).getLog(), 1);
					player.sendMessage("You get some " + ItemAssistant.getItemName(ChopTree.getTree(object).getLog()).toLowerCase() + ".");
					player.getPA().addSkillXP((int) ChopTree.getTree(object).getXP(), Constants.WOODCUTTING);
	...
	public boolean canCut(final int x, final int y) {
		...
		if (ItemAssistant.freeSlots(player) + 1 >= 28) {
			return false;
		}
		if (!Skill.hasRequiredLevel(player, Constants.WOODCUTTING, ChopTree.getTree(object).getLevel(), "woodcutting", "chop this tree")) {
			return false;
		}
		return true;
	}
