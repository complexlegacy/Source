...
public class FamiliarInteraction {
	...
	public static void interactWithFamiliar(Player c, pouchData familiar, String name) {
		String[] chat = { "Null" };
		switch (familiar.npcId) {
		...
		sendFamiliarChat(c, chat, name, familiar.npcId);
	}

	private static void sendFamiliarChat(Player c, String[] message, String familiarName, int familiar) {
		String msg = message[Misc.random(message.length - 1)];
		Dialogues.send(c, new DialogueList(
			new Dialogue[]{
				new Dialogue(DialogueType.NPC_CHAT, Emotion.HAPPY, familiarName, familiar, new String[] {msg})
			}));
	}
